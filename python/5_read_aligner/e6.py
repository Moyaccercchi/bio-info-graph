...
def matchwithindelsfwd(Text, Pattern, d, j):
    ...
    for i in range(0, lenP):
        if (Text[i] != Pattern[i]):
            ...
            a,ainf = matchwithindelsfwd(Text[i:lenP-1], Pattern[i+1:lenP], d, i+j+1)
            b,binf = matchwithindelsfwd(Text[i+1:lenP], Pattern[i+1:lenP], d, i+j+1)
            c,cinf = matchwithindelsfwd(Text[i+2:lenP], Pattern[i+1:lenP-1], d, i+j+1)
...
# j is the current offset at which we start
#   it is only used to form a more informative infostr
def matchwithindelsbwd(Text, Pattern, d):
    ...
    for i in range(lenP-1, -1, -1):
        if (Text[i] != Pattern[i]):
            ...
            a,ainf = matchwithindelsbwd(Text[1:i], Pattern[0:i-1], d)
            b,binf = matchwithindelsbwd(Text[0:i-1], Pattern[0:i-1], d)
            c,cinf = matchwithindelsbwd(Text[0:i-1], Pattern[1:i], d)
...
def isamatch(Pattern, Text, d, position, plen):
    ...
    d,infbwd = matchwithindelsbwd(Text[intextpos:intextpos+inreadpos], Pattern[0:inreadpos], d)
    if d < 0:
    ...
    j = inreadpos+plen+1
    d,inffwd = matchwithindelsfwd(Text[intextpos+inreadpos+plen:intextpos+lenP], Pattern[inreadpos+plen:lenP], d, j)
    if d < 0:
        # (analysis) print("no alignment")
        return [0, 0, ""]
...
def pigeonhole(Pattern, Text, d, suffix_array, plen):
    ...
    for i in range(1, parts+1):
        ...
        if len(sufarr) > 0:
            ...
            while lastfind > -1:
                possible_ret.append([int(sufarr[0:sufarr.find(',')-1]) - j, j])
    ...
    for i in range(0, len(possible_ret)):
        ...
        if ismatch:
            if len(infostr) > 0:
                infostr = " [" + infostr + "]"
            ret.append([scoreofmatch, possible_ret[i][0], infostr])
